...
public class EnviarNotaCreditoCommandHandler : IRequestHandler<EnviarNotaCreditoCommand, ResultadoFacturaElectronica>
{
    private readonly ILogger<EnviarNotaCreditoCommandHandler> _logger;
    ...
    public EnviarNotaCreditoCommandHandler(
        ILogger<EnviarNotaCreditoCommandHandler> logger,
    ...
    public async Task<ResultadoFacturaElectronica> Handle(EnviarNotaCreditoCommand request, CancellationToken cancellationToken)
    {
        _logger.LogInformation("Iniciando proceso de envío de nota de crédito {ConsecutivoDocumento} para tenant {TenantId}",
            request.NotaCredito.ConsecutivoDocumento, request.NotaCredito.TenantId);
        ...
        try
        {
            ...
            ValidarParametrosIniciales(request.NotaCredito);

            // 2. Validar InformacionReferencia (OBLIGATORIA para NC)
            ValidarInformacionReferencia(request.NotaCredito.InformacionReferencia);

            // 3. Mapear a modelo de dominio
            var facturaModel = MapearRequestAFactura(request.NotaCredito);
            ...
            if (!resultadoValidacion.EsValido)
            {
                ...
                _logger.LogError("Nota de crédito {ConsecutivoDocumento} contiene errores de validación: {Errores}",
                    facturaModel.ConsecutivoDocumento, errores);

                return ResultadoFacturaElectronicaExtensions.ConError(
                    "La nota de crédito contiene errores de validación",
            ...
            if (resultadoValidacion.Advertencias.Any())
            {
                ...
                _logger.LogWarning("Nota de crédito {ConsecutivoDocumento} contiene advertencias: {Advertencias}",
            ...
            var clave = _generadorClave.GenerateInvoiceKey(
                ...
                throw new Exception("No se pudo generar la clave de la nota de crédito");
            ...
            var numeroConsecutivo = _generadorConsecutivo.CreaNumeroSecuencia(
                ...
                facturaModel.TipoDocumento!,
                facturaModel.ConsecutivoDocumento) ??
                throw new Exception("No se pudo generar el consecutivo de la nota de crédito");

            _logger.LogInformation("Generada clave {Clave} para nota de crédito {ConsecutivoDocumento}",
                clave, facturaModel.ConsecutivoDocumento);

            // 7. Insertar registro inicial en BD
            registroDocumento = await InsertarRegistroInicial(
                request.NotaCredito.TenantId,
                request.NotaCredito.ExternalDocumentId,
                facturaModel,
                clave,
                numeroConsecutivo,
                resultadoValidacion,
            ...
            var xmlNotaCreditoSinFirmar = generadorDocumentos.CreaXMLFacturaElectronica(facturaModel, clave, numeroConsecutivo);
            ...
            _logger.LogInformation("Documento XML de nota de crédito guardado físicamente para clave {Clave}", clave);
            ...
            _logger.LogInformation("Enviando nota de crédito {Clave} a Hacienda v4.4", clave);
            ...
            // 18. Validar respuesta de Hacienda
            ...
            {
                TenantId = request.NotaCredito.TenantId,
                DocumentId = registroDocumento.Id,
                ExternalDocumentId = request.NotaCredito.ExternalDocumentId,
            ...
            _logger.LogInformation("Nota de crédito {ConsecutivoDocumento} procesada exitosamente con clave {Clave}. Estado Hacienda: {Estado}",
        ...
        {
            _logger.LogError(ex, "Error de validación en nota de crédito {ConsecutivoDocumento}",
                request.NotaCredito.ConsecutivoDocumento);
            return ResultadoFacturaElectronicaExtensions.ConError("Error de validación", [ex.Message]);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error procesando nota de crédito {ConsecutivoDocumento}",
                request.NotaCredito.ConsecutivoDocumento);
            return ResultadoFacturaElectronicaExtensions.ConError("Error interno procesando nota de crédito", [ex.Message]);
        }
    }

    private void ValidarParametrosIniciales(ProcesoNotaCreditoRequest notaCredito)
    {
        if (notaCredito.TenantId == Guid.Empty)
            throw new ArgumentNullException(nameof(notaCredito.TenantId), "El TenantId es requerido");

        if (string.IsNullOrEmpty(notaCredito.EmisorNumeroIdentificacion))
            throw new ArgumentNullException(nameof(notaCredito.EmisorNumeroIdentificacion),
                "La identificación del emisor es requerida");

        if (notaCredito.TipoDocumento != "03")
            throw new ArgumentException("El tipo de documento debe ser '03' para Nota de Crédito");
    }

    private void ValidarInformacionReferencia(InformacionReferenciaRequest? info)
    {
        if (info == null)
    ...
    private Factura MapearRequestAFactura(ProcesoNotaCreditoRequest request)
    {
    ...
    private void ValidarRespuestaHacienda(RecepcionDocumentoResponse respuesta, string clave)
    {
        ...
        {
            case "aceptado":
                _logger.LogInformation("Nota de crédito {Clave} ACEPTADA por Hacienda", clave);
                break;
            case "procesando":
                _logger.LogInformation("Nota de crédito {Clave} en PROCESAMIENTO en Hacienda", clave);
                break;
            case "enviado":
                _logger.LogInformation("Nota de crédito {Clave} ENVIADA a Hacienda", clave);
                break;
            case "rechazado":
                _logger.LogError("Nota de crédito {Clave} RECHAZADA por Hacienda", clave);
                throw new InvalidOperationException($"Nota de crédito rechazada por Hacienda: {clave}");
            case "error":
                _logger.LogError("ERROR procesando nota de crédito {Clave} en Hacienda", clave);
                throw new InvalidOperationException($"Error en Hacienda procesando nota de crédito: {clave}");
        }
    }
    ...
    private async Task<ElectronicInvoice> InsertarRegistroInicial(
        ...
        CancellationToken cancellationToken)
    {
        _logger.LogDebug("Insertando registro inicial en BD para nota de crédito con clave {Clave}", clave);
        ...
        var entidad = new ElectronicInvoice
        {
            ...
            Status = "procesando",
            StatusDetail = "Nota de crédito preparada - Enviando a Hacienda...",
            Clave = clave,
        ...
        var logInicial = new ElectronicDocumentLog
        {
            ...
            Action = "procesando",
            Message = "Nota de crédito preparada y enviando",
        ...
        _logger.LogInformation("Registro inicial insertado con Id {Id} para nota de crédito clave {Clave}", entidad.Id, clave);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var logRespuesta = new ElectronicDocumentLog
        {
            ...
            Action = estado,
            Message = "Respuesta recibida de Hacienda para nota de crédito",
    ...
    private string GenerarDetalleEstado(RecepcionDocumentoResponse respuesta, ResultadoValidacion validacion)
    {
        ...
        {
            "aceptado" => $"Nota de crédito aceptada por Hacienda. Advertencias: {validacion.Advertencias.Count}",
            "procesando" => "Nota de crédito en procesamiento en Hacienda",
            "rechazado" => $"Nota de crédito rechazada por Hacienda: {respuesta.IndicadorEstado}",
            "error" => $"Error procesando nota de crédito: {respuesta.IndicadorEstado}",
            "enviado" => "Nota de crédito enviada a Hacienda",
